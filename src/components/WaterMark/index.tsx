import React, {useState, useEffect, useContext, CSSProperties, ReactNode, FC} from 'react';
...
const WaterMark:FC<WaterMarkProps> = (props) =>  {
  const {
    ...
    prefixCls = "water-mark",
  } = props;

  const {getPrefixCls} = useContext(ConfigProvider.ConfigContext);
  const currentPrefixCls = getPrefixCls("yanwen-layout-watermark", 'water-mark');

  const wrapperCls = classNames(`${currentPrefixCls}-wrapper`, className);
  const markCls = classNames(`${currentPrefixCls}-mark`, markClassName);
  const waterMarkCls = classNames(currentPrefixCls, markClassName);
  ...
  useEffect(() => {
    ...
    if(ctx) {
      ...
      console.error("canvas context is not available, 当前的环境不支持canvas");
    }
